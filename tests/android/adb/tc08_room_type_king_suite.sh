...
echo "[TC08] Search hotels in San Francisco (check-in 2024-04-01, check-out 2024-04-04), find Union Square Premier, book it with King Suite room type, guest name Suite Guest, email suite@test.com, phone 15550008888"
clear_db
...
pull_db_cat
RC=$(qdb "SELECT COUNT(*) FROM bookings WHERE booking_type='hotel' AND item_id='hotel_005' AND item_name='Union Square Premier' AND user_name='Suite Guest' AND user_email='suite@test.com' AND user_phone='15550008888' AND room_type='King Suite' AND status='confirmed';")
[ "$RC" -gt 0 ] && PASS=1
