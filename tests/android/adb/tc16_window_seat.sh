...
pull_db_cat
RC=$(qdb "SELECT COUNT(*) FROM bookings WHERE booking_type='flight' AND item_id='flight_004' AND user_name='Window User' AND user_email='win@fly.com' AND user_phone='15550001616' AND extras LIKE '%seat_preference%window%' AND status='confirmed';")
[ "$RC" -gt 0 ] && PASS=1
